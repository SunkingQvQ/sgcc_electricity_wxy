...
class DataFetcher:
    ...
    def _get_webdriver(self):
        ...
        self._inject_fingerprint_spoofing(driver)
        
        return driver

    def _inject_fingerprint_spoofing(self, driver):
        """注入浏览器指纹伪装脚本，防止容器环境被检测。
        ...
        try:
            fingerprint_script = """
                    ...
                    value: {
                        ...
                        getHighEntropyValues: function() {
                            return Promise.resolve({
                                ...
                                browserFullVersion: '120.0.6099.130'
                            });
                        }
                    },
                    enumerable: true,
                    ...
                    if (type === '2d') {
                        ...
                        context.fillText = function() {
                            ...
                            if (args.length > 0 && typeof args[0] === 'string') {
                                args[0] = args[0].replace(/canvas fingerprint/, '');
                            }
                ...
                Object.defineProperty(window, 'chrome', {
                    value: {
                        ...
                        tabs: {query: function() {}},
                        extension: {sendMessage: function() {}}
                    },
                    enumerable: true
                });
            """
            ...
            logging.info("浏览器指纹伪装脚本已注入")
        except Exception as e:
            logging.warning(f"指纹伪装注入失败: {e}")
    ...
    def _login(self, driver, phone_code = False):
        ...
        self._wait_for_page_full_load(driver)
        ...
        # 增加判空校验便于测试备用方案
            ...
            if self._check_risk_control(driver, error):
                logging.error("检测到账号风控（RK001），立即停止本轮登录尝试，避免账号被封")
                return False
            ...
            if error is None:
                ...
                if captcha_passed:
                    ...
                    if driver.current_url != LOGIN_URL:
                        ...
                        error = self._get_error_message(driver, "//div[@class='errmsg-tip']//span")
                        if self._check_risk_control(driver, error):
                            logging.error("验证码通过后检测到账号风控，立即停止")
                            return False
                        if error:
                    ...
                    error = self._get_error_message(driver, "//div[@class='errmsg-tip']//span")
                    if self._check_risk_control(driver, error):
                        logging.error("验证码重试后检测到账号风控，立即停止")
                        return False
    ...
    def _check_risk_control(self, driver, error_message: str = None) -> bool:
        """检测是否触发账号风控（RK001）。
        
        返回 True 表示检测到风控，应立即停止登录尝试。
        """
        risk_keywords = ["RK001", "风控", "风险", "账号异常", "登录受限", "暂时无法登录", "安全验证"]
        
        if error_message:
            for keyword in risk_keywords:
                if keyword in error_message:
                    logging.error(f"检测到风控关键词: {keyword}")
                    return True
        ...
        try:
            ...
            for keyword in risk_keywords:
                if keyword in body_text:
                    logging.error(f"从页面正文检测到风控关键词: {keyword}")
                    return True
        except Exception:
            pass
        ...
        try:
            ...
            if "RK001" in page_source:
                logging.error("从页面源码检测到 RK001 风控")
                return True
        except Exception:
            pass
    ...
    def _wait_for_page_full_load(self, driver, timeout: int = 30):
        """等待页面完整加载，包括验证码 iframe 和图片资源。
        
        防止 window.stop() 导致的资源加载不完整问题。
        """
        try:
            WebDriverWait(driver, timeout).until(
                lambda d: d.execute_script("return document.readyState === 'complete'")
            )
            logging.info("页面 DOMContentLoaded 完成")
            ...
            logging.info("页面资源加载完成")
        except Exception as e:
            logging.warning(f"页面完整加载等待超时: {e}")
