...
def _detect_captcha_type_js(driver: WebDriver) -> str:
    ...
    关键改进：严格过滤隐藏 DOM（预加载的验证码节点通常 top: -1000000px, 高度为0）
    """
    try:
        result = driver.execute_script("""
            ...
            function hasVisibleWidget() {
                var widgets = document.querySelectorAll(
                    '.tencent-captcha-dy__warp, .tencent-captcha-dy__wrapper, ' +
                    '.tencent-captcha__wrapper, .tencent-captcha-dy__body-wrap, #tCaptchaDyContent'
                );
            ...
            if (!hasVisibleWidget()) return 'unknown';
