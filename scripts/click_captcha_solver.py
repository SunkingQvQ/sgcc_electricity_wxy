...
class ClickCaptchaSolver:
    ...
    def _find_all_icons(self, icon_uris: List[str], main_uri: str,
        ...
        prompt = (
            f"这是一个腾讯点选验证码任务。\n"
            f"大图尺寸：{main_width}×{main_height}像素，是一个图标网格。\n"
            f"请找到3个参考图标(A, B, C)各自在大图网格中的准确位置。\n\n"
            f"匹配规则：\n"
            f"1. 形状是最重要的匹配依据，颜色可能有轻微差异但形状必须完全一致\n"
            f"2. 空心/实心是关键区分点，线条粗细和方向也要匹配\n"
            f"3. 图标可能有轻微旋转（±30度），但整体形状不变\n"
            f"4. 大图中的图标大小可能与参考图标不完全一致，但形状特征保持不变\n"
            f"5. 每个图标在大图中只出现一次，找到最匹配的位置\n\n"
            f"输出要求：\n"
            f'- 输出JSON格式：{{"coords":[[xA,yA],[xB,yB],[xC,yC]]}}\n'
            f"- x、y为图标中心的像素坐标（0~{main_width}，0~{main_height}）\n"
            f"- 必须严格按照A、B、C的顺序输出坐标\n"
            f"- 如果某个图标在大图中找不到，该位置输出[0,0]\n"
            f"- 只输出JSON，不要任何解释文字"
        )
